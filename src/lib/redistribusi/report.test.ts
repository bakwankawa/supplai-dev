...
describe("createRedistribusiReport", () => {
  ...
  it("includes the dashboard tooltip glossary in both report versions", () => {
    const a = analisis("beras", "seimbang")
    for (const pembaca of PEMBACA) {
      const text = extractPdfText(createRedistribusiReport(a, pembaca))
      expect(text).toContain("Daftar Isi")
      expect(text).toContain("Glosarium")
      expect(text).toContain("Wilayah asal (surplus)")
      expect(text).toContain("Cara membaca matriks rute")
      expect(text).toContain("Kecukupan GPM")
      expect(text).toContain("Kelompok IKP")
      expect(text).toContain("Asal-usul angka")
      expect(text).toContain("Ditulis model bahasa")
    }
  })

  it("marks every report with its generator and SupplAI attribution", () => {
    const dibuat = new Date("2026-09-08T16:30:00.000Z")
    for (const pembaca of PEMBACA) {
      const text = extractPdfText(createRedistribusiReport(analisis("beras", "seimbang"), pembaca, dibuat, "Robertus Rotama"))
      expect(text).toContain("Dibuat oleh Robertus Rotama")
      expect(text).toContain("Generated by SupplAI")
    }
  })
