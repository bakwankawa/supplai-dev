...
export function PredictionNarrative({ analysis: a }: { analysis: PredictionAnalysis }) {
  return (
    <section aria-labelledby="prediction-narrative-title" className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 space-y-5 shadow-none">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-4">
        <div>
          <h2 id="prediction-narrative-title" className="flex items-center gap-2 text-lg font-bold text-slate-900"><FileText className="h-5 w-5" />Penjelasan Hasil Prediksi</h2>
      ...
      <div className="space-y-2 text-sm leading-7 text-slate-700">
        <p>{a.narrative}</p>
        <p><span className="font-semibold text-slate-800">Faktor yang perlu diverifikasi:</span> {a.factorNarrative}</p>
      </div>
      ...
      <div className="flex items-start gap-2 rounded-xl border border-amber-100 bg-amber-50/60 p-3 text-xs leading-5 text-amber-900">
        ...
        <p>{a.signal}. MAPE (Mean Absolute Percentage Error atau Rata-rata Persentase Kesalahan Absolut) mengukur rata-rata kesalahan historis satu bulan, bukan kepastian prediksi. Acuan data: {a.baselineDate ? monthLabel(a.baselineDate) : "belum tersedia"}; harga lapangan perlu diperbarui sebelum keputusan. Narasi memakai aturan penjelasan hasil ML, bukan model bahasa generatif.</p>
      </div>
